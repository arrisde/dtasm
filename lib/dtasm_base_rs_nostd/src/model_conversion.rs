...
pub fn collect_var_types(md: &MD::ModelDescription) -> BTreeMap<i32, DtasmVarType> {
    let model_vars = &md.variables;
    let mut var_types: BTreeMap<i32, DtasmVarType> = BTreeMap::new();
...
pub fn add_var_types(md: &MD::ModelDescription, var_types: &mut BTreeMap<i32, DtasmVarType>) {
    let model_vars = &md.variables;
